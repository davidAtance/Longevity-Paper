...
##############################################################################
#Here we construct the correlations plot
##############################################################################
library(PerformanceAnalytics)
chart.Correlation(datosh2, histogram = TRUE, method = "pearson")

library(psych)
corPlot(datosh2, cex = 1.2)
...
library(corrplot)
datos2 <- rbind.data.frame(datosh2, datosm2)
colnames(datos2) <- c("e0t", "e65t", "Mt", "G0t", "G65t", "s0t", "s65t", "y50%,0t", "y75%,65t")


cordatos2 <- cor(datos2)

corrplot(cordatos2, method="circle")
corrplot(cordatos2, method="color")
corrplot(cordatos2, method="number")

col <- colorRampPalette(c("#BB4444", "#EE9988", "#FFFFFF", "#77AADD", "#4477AA"))
col23 <- c("#FF0000", "#FF9191", "#FFAAAA", "#FFE2E2", "#FFFFFF", "#FFFFFF", "#E2E2FF", "#AAAAFF", "#7171FF", "#0000FF")
corrplot(cordatos2, method="color", addCoef.col = "black", type="upper", tl.col="black", tl.srt=45,
         sig.level = 0.001, diag= F, insig = "blank", col = col23)
...
#Ver que pa?s ocupa cada cluster
...
class(world)
#Latitude y Longuitud unida con cada pa?s
...
centers2030 <- rbind(centers2030, centers[centers$name == "Cura?ao",])
...
centers2000 <- rbind(centers2000, centers[centers$name == "C?te d'Ivoire",])
...
centers2025 <- rbind(centers2025, centers[centers$name == "S?o Tom? and Principe",])
